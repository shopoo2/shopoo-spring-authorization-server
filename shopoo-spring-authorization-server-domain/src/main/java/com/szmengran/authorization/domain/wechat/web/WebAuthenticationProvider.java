...
public class WebAuthenticationProvider extends AbstractUserDetailsAuthenticationProvider {
    ...
    public WebAuthenticationProvider(WechatRepository wechatRepository, UserDetailsService userDetailsService, WechatProperties wechatProperties, MiniProgramRepository miniProgramRepository, OAuth2TokenGenerator<? extends OAuth2Token> tokenGenerator) {
    ...
    public Authentication authenticate(final Authentication authentication) throws AuthenticationException {
        WebAuthorizationToken webAuthorizationToken = (WebAuthorizationToken) authentication;
        OAuth2ClientAuthenticationToken clientPrincipal = OAuth2AuthenticationProviderUtils.getAuthenticatedClientElseThrowInvalidClient(webAuthorizationToken);
        RegisteredClient registeredClient = clientPrincipal.getRegisteredClient();
    
        UsernamePasswordAuthenticationToken usernamePasswordAuthenticationToken = checkAndRegister(registeredClient, webAuthorizationToken.getCode());
    
        Set<String> scopes = Optional.ofNullable(webAuthorizationToken.getScopes()).orElse(registeredClient.getScopes());
        DefaultOAuth2TokenContext.Builder tokenContextBuilder = DefaultOAuth2TokenContext.builder().registeredClient(registeredClient).principal(usernamePasswordAuthenticationToken).authorizationServerContext(AuthorizationServerContextHolder.getContext()).authorizedScopes(scopes).authorizationGrantType(WebAuthorizationToken.GRANT_TYPE);
    ...
    public boolean supports(final Class<?> authentication) {
        return WebAuthorizationToken.class.isAssignableFrom(authentication);
    }
}
